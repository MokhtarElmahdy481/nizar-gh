...
export default function TeamsSection({doctors}:{doctors: Doctor[]}) {
          ...
          {doctors.map((item, index) => (
            <div className="mb-10" key={index}>
              <Link
                ...
                key={index}
              >
                <Image width={100} height={100} alt="team/img" src={item.image} />
                <div className="teamDetails">
                  <p className={`m-0 p-0 mb-2 teamName `}>{item.name}</p>
                  <p className={`m-0 p-0 mb-1 teamTitle `}>{item.des}</p>
                </div>
              </Link>
            </div>
          ))}
